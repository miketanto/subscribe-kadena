/**
 * From https://github.com/jaredhanson/passport-http-bearer/blob/master/lib/strategy.js
 */
...
Strategy.prototype._challenge = function (code, desc, uri) {
  ...
  if (this._scope) challenge += `, scope="${this._scope.join(' ')}"`
  if (code) challenge += `, error="${code}"`
  if (desc && desc.length) challenge += `, error_description="${desc}"`
  if (uri && uri.length) challenge += `, error_uri="${uri}"`
  return challenge
}
