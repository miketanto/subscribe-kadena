 ...
 */
 ...
 var prepareExecCmd = function(keyPairs=[], nonce=new Date().toISOString(), pactCode,
                               envData, meta=mkMeta("","",0,0,0,0), networkId=null,coSigs = []) {
   ...
   //The whole suite of signers is needed up till here
   var cmdJSON = {
     networkId: networkId,
     payload: {
       exec: {
   ...
   //This is the command that needs to be kept for withdrawal
 ...
 export const getRawCmd = function(keyPairs=[], nonce=new Date().toISOString(), pactCode,
                               envData, meta=mkMeta("","",0,0,0,0), networkId=null,coSigs = []) {
...
export const fetchSendPreparedCmd = async function(preparedCmd, apiHost){
  let res = fetch(`${apiHost}/api/v1/send`, mkReq(mkPublicSend(asArray(preparedCmd))));
  return parseRes(res)
} 
 ...
 var mkMeta = function(sender, chainId, gasPrice, gasLimit, creationTime, ttl) {
   console.debug("Entered mkMeta Function")
   enforceType(sender, "string", "sender");
   console.debug("sender type matched")
   enforceType(chainId, "string", "chainId");
   console.debug("chainId type matched")
   enforceType(gasPrice, "number", "gasPrice");
   console.debug("gasPrice type matched")
   enforceType(gasLimit, "number", "gasLimit");
   console.debug("gasLimit type matched")
   enforceType(creationTime, "number", "creationTime");
   console.debug("creationTime type matched")
   enforceType(ttl,  "number", "ttl");
   console.debug("ttl type matched")
   return {
